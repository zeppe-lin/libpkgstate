...
Current request records use fixed eight-byte magic **ZLSPRQST** followed by
big-endian schema version 2. The decoder also admits canonical version-1 records
emitted by libpkgstate 2.5.0; the public encoder emits only version 2.
...
Current receipt records use fixed eight-byte magic **ZLSPRCPT** followed by
big-endian schema version 2. Canonical 2.5.0 version-1 records remain decodeable;
new encodings never use the legacy textual magic.
