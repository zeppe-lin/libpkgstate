...
It does not answer what a recipe text says, what should be selected, how to build
...
Build authority supplies a resolved build-input set and build result. Planner and
image authorities supply candidate, artifact, and manifest identities.
Application authority supplies completed target effects. The state application
adapter validates those authorities together and creates a complete installation
receipt.
...
application evidence: object kind, mode, owner, timestamp, kind-specific content,
optional hard-link topology, active origin, and rejected-object provenance. State
does not rediscover these facts from the target filesystem.
...
selects one. Storage version 2 is not a reinterpretation of the previous format.
A separate migration utility is required for any old storage.
