...
grep -F "version: '2.4.0'" "$root/meson.build" >/dev/null || fail 'Meson version is not 2.4.0'
grep -F 'PROJECT_NUMBER         = 2.4.0' "$root/Doxyfile" >/dev/null || fail 'Doxygen version is not 2.4.0'
...
grep -F '2.4.0' "$root/HISTORY.md" >/dev/null || fail 'history omits 2.4.0'
