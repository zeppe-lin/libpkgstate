...
grep -F 'canonical generation-v3 storage' "$root/HISTORY.md" >/dev/null || fail 'history omits retained storage authority'
grep -F "'publication_codec.cpp'" "$root/src/meson.build" >/dev/null || fail 'core source omits publication codec'
grep -F "'canonical_generation_store.cpp'" "$root/src/meson.build" >/dev/null || fail 'core source omits generation backend'
