...
grep -F "version: '2.1.0'" "$root/meson.build" >/dev/null || fail 'Meson version is not 2.1.0'
grep -F 'PROJECT_NUMBER         = 2.1.0' "$root/Doxyfile" >/dev/null || fail 'Doxygen version is not 2.1.0'
...
grep -F "version: '>=1.0.0'" "$root/meson.build" >/dev/null || fail 'native 1.0 authority floor is missing'
...
grep -F '2.1.0' "$root/HISTORY.md" >/dev/null || fail 'history omits 2.1.0'
...
grep -F '`libpkgstate-apply` advances to soversion 3' "$root/HISTORY.md" >/dev/null || fail 'history omits apply ABI transition'
