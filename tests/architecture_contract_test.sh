...
grep -F 'Concrete paths, directories, locks, selectors, durability, recovery, and diagnostic mechanisms are provider authority' "$root/docs/architecture.md" >/dev/null || fail 'provider placement is undocumented'
...
for path in include/libpkgstate/generation_codec.h src/generation_codec.cpp; do [ -s "$root/$path" ] || fail "state protocol absent: $path"; done
grep -F 'encode_generation_snapshot' "$root/include/libpkgstate/generation_codec.h" >/dev/null || fail 'generation codec API absent'
