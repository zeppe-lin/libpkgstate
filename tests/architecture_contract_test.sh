...
for path in source_adapter build_adapter adapter apply_adapter include/libpkgstate-source include/libpkgstate-build include/libpkgstate-plan include/libpkgstate-apply; do [ ! -e "$root/$path" ] || fail "foreign adapter retained: $path"; done
...
grep -F 'publication codec and canonical generation store remain here' "$root/docs/architecture.md" >/dev/null || fail 'persistence placement is undocumented'
