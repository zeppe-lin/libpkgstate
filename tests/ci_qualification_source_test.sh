...
for v in 'GCC shared' 'GCC static' 'Clang shared' 'Clang static' 'GCC release' 'address,undefined' 'meson==1.6.1' '--wrap-mode=nofallback'; do grep -F -- "$v" "$root/.github/workflows/ci.yml" "$root/ci/configure-and-test.sh" >/dev/null || { echo "missing CI contract: $v" >&2; exit 1; }; done
