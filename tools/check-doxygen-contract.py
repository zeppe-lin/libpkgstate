...
def belongs_to_header(node: dict[str, Any], header: Path) -> bool:
    ...
    # Clang omits the file name for declarations in the main input. Included
    # declarations still carry an includedFrom chain.
    ...
    for node in nodes:
        ...
        if kind in {"CXXRecordDecl", "ClassTemplateDecl"}:
            if not belongs_to_header(node, header):
                continue
            default_access = (
                "public" if node.get("tagUsed") == "struct" else "private"
            )
            issues.extend(
                inspect_nodes(node.get("inner", []), header, default_access)
            )
            if (
        ...
        if kind in CONTAINER_KINDS:
            issues.extend(
                inspect_nodes(node.get("inner", []), header, current_access)
            )
            continue
